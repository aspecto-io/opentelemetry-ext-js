...
describe("plugin-aws-sdk", () => {
  ...
  beforeEach(() => {
    ...
    context.setGlobalContextManager(
      (contextManager.enable() as unknown) as ContextManager
    );
  });
  ...
  it("adds proper number of spans with correct attributes", async (done) => {
    ...
    expect(spanCreateBucket.attributes[AttributeNames.AWS_PARAMS]).toBe(
    ...
    expect(spanPutObject.attributes[AttributeNames.AWS_PARAMS]).toBe(
      '{"Bucket":"aws-test-bucket","Key":"aws-test-object.txt","Body":"Hello World!"}'
    );
    expect(spanPutObject.attributes[AttributeNames.AWS_SIGNATURE_VERSION]).toBe(
      "s3"
    );
