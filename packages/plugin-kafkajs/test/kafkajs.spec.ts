...
describe("plugin-kafkajs", () => {
  ...
  describe("producer", () => {
    const expectKafkaHeadersToMatchSpanContext = (
      ...
      ).toStrictEqual(span.spanContext.traceId);
      expect(
        kafkaMessage.headers[DummyPropagation.SPAN_CONTEXT_KEY]
      ).toStrictEqual(span.spanContext.spanId);
    };
    ...
    describe("successful send", () => {
      ...
      it("simple send create span with right attributes, pass return value correctly and propagate context", async () => {
        ...
        expect(span.attributes[AttributeNames.MESSAGING_SYSTEM]).toStrictEqual(
          "kafka"
        );
        expect(
          span.attributes[AttributeNames.MESSAGING_DESTINATIONKIND]
        ).toStrictEqual("topic");
        expect(
          span.attributes[AttributeNames.MESSAGING_DESTINATION]
        ).toStrictEqual("topic-name-1");
    ...
    describe("failed send", () => {
      ...
      it("error in send create failed span", async () => {
        ...
        expect(span.status.message).toStrictEqual(
    ...
    describe("producer hook successful", () => {
      ...
      it("producer hook add span attribute with value from message", async () => {
        ...
        expect(spans.length).toBe(1);
        const span = spans[0];
        expect(span.attributes["attribute-from-hook"]).toStrictEqual(
  ...
  describe("consumer", () => {
    ...
    describe("successful consumer hook", () => {
      ...
      it("consume hook adds attribute to span", async () => {
        ...
        expect(spans.length).toBe(1);
        const span = spans[0];
        expect(span.attributes["attribute key from hook"]).toStrictEqual(
    ...
    describe("successful eachBatch", () => {
      ...
      it("consume eachBatch create span with expected attributes", async () => {
        ...
        spans.forEach((span) => {
          ...
          expect(span.status.code).toStrictEqual(CanonicalCode.OK);
          expect(
            span.attributes[AttributeNames.MESSAGING_SYSTEM]
          ).toStrictEqual("kafka");
          expect(
            span.attributes[AttributeNames.MESSAGING_DESTINATIONKIND]
          ).toStrictEqual("topic");
        ...
        expect(msg1Span.parentSpanId).toStrictEqual(
          recvSpan.spanContext.spanId
        );
        expect(msg2Span.parentSpanId).toStrictEqual(
          recvSpan.spanContext.spanId
        );
      });
    });
  ...
  describe("context propagation", () => {
    ...
    it("context injected in producer is extracted in consumer", async () => {
      ...
      expect(consumerSpan.spanContext.traceId).toStrictEqual(
        producerSpan.spanContext.traceId
      );
      expect(consumerSpan.parentSpanId).toStrictEqual(
    ...
    it("context injected in producer is extracted as links in batch consumer", async () => {
      ...
      expect(processingSpan.spanContext.traceId).toStrictEqual(
        receivingSpan.spanContext.traceId
      );
      expect(processingSpan.parentSpanId).toStrictEqual(
        receivingSpan.spanContext.spanId
      );
      expect(processingSpan.links.length).toBe(1);
      expect(processingSpan.links[0].context.traceId).toStrictEqual(
        producerSpan.spanContext.traceId
      );
      expect(processingSpan.links[0].context.spanId).toStrictEqual(
        producerSpan.spanContext.spanId
      );

      // receiving span should start a new trace
      expect(receivingSpan.parentSpanId).toBeUndefined();
      expect(receivingSpan.spanContext.traceId).not.toStrictEqual(
        producerSpan.spanContext.traceId
      );
    });
  });
