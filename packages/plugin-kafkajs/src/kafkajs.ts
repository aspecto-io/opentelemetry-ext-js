...
export class KafkaJsPlugin extends BasePlugin<typeof kafkaJs> {
  ...
  protected patch() {
    ...
    shimmer.wrap(
      this._moduleExports?.Kafka?.prototype,
      "producer",
      this._getProducerPatch.bind(this)
    );
    shimmer.wrap(
      this._moduleExports?.Kafka?.prototype,
      "consumer",
      this._getConsumerPatch.bind(this)
    );
  ...
  private _getConsumerPatch(original: (...args: unknown[]) => Producer) {
    ...
    return function (...args: unknown[]): Consumer {
      ...
      shimmer.wrap(
        newConsumer,
        "run",
        thisPlugin._getConsumerRunPatch.bind(thisPlugin)
      );
      return newConsumer;
    };
  }
  ...
  private _getProducerPatch(original: (...args: unknown[]) => Producer) {
    ...
    return function (...args: unknown[]): Producer {
      ...
      shimmer.wrap(
        newProducer,
        "sendBatch",
        thisPlugin._getProducerSendBatchPatch.bind(thisPlugin)
      );
      shimmer.wrap(
        newProducer,
        "send",
        thisPlugin._getProducerSendPatch.bind(thisPlugin)
      );
      return newProducer;
    };
  }
    ...
    return function (config?: ConsumerRunConfig): Promise<void> {
      if (config?.eachMessage) {
        shimmer.wrap(
          config,
          "eachMessage",
          thisPlugin._getConsumerEachMessagePatch.bind(thisPlugin)
        );
      }
      if (config?.eachBatch) {
        shimmer.wrap(
          config,
          "eachBatch",
          thisPlugin._getConsumerEachBatchPatch.bind(thisPlugin)
        );
      }
    ...
    return function (payload: EachBatchPayload): Promise<void> {
      ...
      return thisPlugin._tracer.withSpan(receivingSpan, () => {
        const spans = payload.batch.messages.map((message: KafkaMessage) => {
          ...
          const spanContext: SpanContext = getExtractedSpanContext(
            propagatedContext
          );
  ...
  private _getProducerSendBatchPatch(
    original: (batch: ProducerBatch) => Promise<RecordMetadata[]>
  ) {
    const thisPlugin = this;
    return function (batch: ProducerBatch): Promise<RecordMetadata[]> {
      const spans: Span[] = batch.topicMessages.flatMap((topicMessage) =>
        topicMessage.messages.map((message) =>
          thisPlugin._startProducerSpan(topicMessage.topic, message)
        )
      );
  ...
  private _getProducerSendPatch(
    original: (record: ProducerRecord) => Promise<RecordMetadata[]>
  ) {
    const thisPlugin = this;
